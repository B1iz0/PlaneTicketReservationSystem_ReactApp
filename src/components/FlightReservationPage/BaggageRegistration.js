...
const BaggageRegistration = (
  ...
  const onBaggageWeightChange = (event) => {
    ...
    } else if (event.target.value > selectedFlight?.airplane?.onePersonBaggageLimitInKilograms) { 
      setIsBaggageWeightValid(false);
      setBaggageWeightHelperText(`Value must be less than or equal to ${selectedFlight?.airplane?.onePersonBaggageLimitInKilograms} Kg`);
            ...
            <Grid item lg>
              <TextField 
                ...
                InputProps={{
                  ...
                  inputProps: { min: 0, max: selectedFlight?.airplane?.onePersonBaggageLimitInKilograms },
                }}
              />
            </Grid>
            <Grid item lg={8}>
